...
const gameSlice = createSlice({
  ...
  reducers: {
    ...
    endGame: (state) => {
      state.gameOver = true;
      state.score +=
        (state.settings.time - state.elapsedTime) *
        (state.settings.time / config.time) *
        10;
    },
    resetGame: (state) => {
      ...
      state.flippedIds = [];
      state.cards = shuffle(
        createCards(animalEmojis.slice(0, state.settings.cards)),
      );
      state.elapsedTime = 0;
      resetScore();
    },
    ...
    flipCard: (state, action: PayloadAction<number>) => {
      state.cards = state.cards.map((card) =>
        card.id === action.payload ? { ...card, flipped: true } : card,
      );
      state.flippedIds.push(action.payload);
    },
    flipCardsBack: (state) => {
      state.cards = state.cards.map((card) =>
        state.flippedIds.includes(card.id) ? { ...card, flipped: false } : card,
      );
      state.flippedIds = [];
    },
